        ...
        if (!this.typing) {
            ...
            input.addEventListener('keypress', event => {
                ...
                if (charTyped === this.quote.charAt(this.index)) {
                    ...
                    // Change text color to red if errors occur
                    if(this.errorIndex > 0) {
                        _errors.innerHTML = `<span id="errors-active" class="d-block stats"> ${this.errorIndex} </span>`
                    }
...
/// Select result color
const selectResultColor = (wordsPerMinute) => {
    const wpm = Number(wordsPerMinute);
    if(wpm > 5 && wpm < 20) {
        return `<li style="color:#eb4841">${savedWPM}</li>`;
    } else if(wpm > 20 && wpm < 40) {
        return `<li style="color:#f48847">${savedWPM}</li>`;
    } else if(wpm > 40 && wpm < 60) {
        return `<li style="color:#ffc84a">${savedWPM}</li>`;
    } else if(wpm > 60 && wpm < 80) {
        return `<li style="color:#a6c34c">${savedWPM}</li>`;
    } else if(wpm > 80) {
        return `<li style="color:#4ec04e">${savedWPM}</li>`;
    } else {
        return `<li>${savedWPM}</li>`;
    }
};

// Save last wpm result to Local storage
const savedWPM = localStorage.getItem('WPM') || 0;
select('#result').innerHTML = selectResultColor(savedWPM);
